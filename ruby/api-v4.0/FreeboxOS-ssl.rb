...
class Rest
	...
	def get method
		...
		JSON::parse(self.execute(:get, @url + method, @headers))
	end
	...
	def post method, args=nil
		...
		case args
			when nil
				...
				JSON::parse(self.execute(:post, @url + method, @headers, args))
		end
	end
	
	# enable extra options for HTTPS URL
	def execute method, url, headers=nil, args=nil
		# https://www.rubydoc.info/gems/rest-client/RestClient/Request
		puts "Rest::execute(#{method.to_s})"
		case method
		when :get
			RestClient::Request.execute(
					...
					url: url,
					headers: headers,
					verify_ssl: false   # FIXME : should support Freebox Root CA file
				);
		when :post
			pp args
			puts args.to_json
...
class Login < Core
	...
	def authorize device=nil
		device=`hostname`.strip unless device
		args = {
